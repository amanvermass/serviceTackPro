...
export default function TeamManagement() {
  ...
  const fetchTeam = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/teams?${params.toString()}`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        const apiMembers: TeamMember[] = items.map((item: any) => {
          ...
          const assignedProjectsRaw = Array.isArray(item.assignedProjects)
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/api/teams/send-email-otp`, {
        method: "POST",
        headers: {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/api/teams/verify-email-otp`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  const handleResendOtp = async () => {
    try {
      ...
      await fetch(`${baseUrl}/api/teams/send-email-otp`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  const filtered = useMemo(() => {
    ...
    return members.filter((m) => {
      ...
      const matchesRole =
        roleFilter === "all" || m.role.toLowerCase().includes(roleFilter);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const url = `${baseUrl}/api/teams/export${params.toString() ? `?${params.toString()}` : ""}`;
  ...
  const addMemberSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Check email verification for new members
    if (!editingMember && !isEmailVerified) {
      return;
    }
      ...
      <main className="flex-grow w-full px-10 py-[2vh]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-text-secondary">
                Manage team members, roles, and project assignments with
        ...
        <div className="card mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            {/* Role Filter */}
            <div>
              <label htmlFor="roleFilter" className="input-label">
                Role
              </label>
              <select
                id="roleFilter"
            ...
            {/* Status Filter */}
            <div>
              <label htmlFor="statusFilter" className="input-label">
                Availability
              </label>
              <select
                id="statusFilter"
                      ...
                      >
                        ...
                        <td>
                          <div className="space-y-1">
                            ...
                            {(m.assignedProjects?.length ?? m.projects.length) >
                              2 && (
                              <p className="text-xs text-text-secondary">
                                +
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="badge badge-primary">
                      {selectedMember.workload}% Capacity
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div id="addMemberModal" className="modal-overlay">
            <div className="modal-content">
              ...
              <form className="p-6 space-y-6" onSubmit={addMemberSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {isEmailVerified && (
                        <span className="badge badge-success flex items-center gap-1 px-2! py-0.5! text-xs!">
                          <svg
                            className="w-3"
                            fill="currentColor"
                            viewBox="0 0 20 20"
                    ...
                    <div className="flex gap-2 items-end">
                      <div className="flex-1 relative">
                        <input
                          ...
                          className={`input ${
                            isEmailVerified
                              ? "bg-success-50 border-success text-text-primary"
                              : ""
                          }`}
                      ...
                      {!isEmailVerified && !isOtpSent && (
                        <button
                          ...
                          className="btn btn-outline h-10 px-4 whitespace-nowrap disabled:opacity-50"
                        >
                          Verify
                        </button>
                      )}
                  ...
                  <div className="rounded-lg border border-primary-200 bg-primary-50 p-4 space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-text-secondary mb-3">
                        We've sent a verification code to {newMember.email}
                      </p>
                      <div className="flex gap-2 justify-between">
                        {otpDigits.map((digit, index) => (
                          <input
                            ...
                            className="w-full h-12 text-center text-lg font-semibold border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-smooth"
                            placeholder="-"
                          />
                        ))}
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="memberPhone" className="input-label">
                    ...
                    {[
                      ...
                      >
                        <input
                          ...
                          defaultChecked={
                            editingMember?.permissions.includes(perm) ?? false
                          }
                        />
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    type="submit"
                    disabled={!editingMember && !isEmailVerified}
