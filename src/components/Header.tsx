  ...
  const handleNotificationClick = async (notification: NotificationType) => {
    try {
      if (!notification.read) {
        ...
        const response = await fetch(
          `${baseUrl}/api/notifications/read/${notification.id}`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      <header className="bg-surface border-b border-border fixed top-0 left-0 right-0 z-50 shadow-sm w-full max-w-[100vw]">
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <path
                  d="M16 8L8 12V20C8 23.314 11.134 26 16 26C20.866 26 24 23.314 24 20V12L16 8Z"
                  fill="white"
                />
                <circle cx="16" cy="16" r="3" fill="#F59E0B" />
              </svg>
              <span className="font-heading font-bold text-[clamp(1rem,2vw,1.25rem)] text-text-primary">
                Domain Manager Pro
              </span>
            </div>
            ...
            <div className="hidden md:flex items-center gap-6">
              <nav className="flex items-center gap-1">
                <Link
                  href="/dashboard"
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-smooth ${isActive("/dashboard")}`}
                >
                  Dashboard
                </Link>
                {permissions.clients !== false && (
                  <Link
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-smooth ${isActive("/client-management")}`}
                  >
                    Clients
                  </Link>
                )}
                ...
                {permissions.domains !== false && (
                  <div className="relative group">
                    <button
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-smooth flex items-center gap-1 cursor-default ${isParentActive("/domain-management")}`}
                    >
                      Domains
                      <svg
                    ...
                    <div className="absolute top-full left-0 mt-1 w-48 bg-surface border border-border rounded-lg shadow-lg py-1 z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-left">
                      <Link
                        ...
                        className={`block px-4 py-2 text-sm transition-colors ${pathname === "/domain-management" ? "bg-primary-50 text-primary-700" : "text-text-secondary hover:bg-surface-hover hover:text-text-primary"}`}
                      >
                        All Domains
                      </Link>
                      <Link
                        href="/domain-management/vendors"
                        className={`block px-4 py-2 text-sm transition-colors ${pathname === "/domain-management/vendors" ? "bg-primary-50 text-primary-700" : "text-text-secondary hover:bg-surface-hover hover:text-text-primary"}`}
                      >
                        Vendors
                      </Link>
                    </div>
                ...
                {permissions.hosting !== false && (
                  <Link
                    href="/hosting-management"
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-smooth ${isActive("/hosting-management")}`}
                  >
                    Hosting
                  </Link>
                )}
                {permissions.maintenance !== false && (
                  <Link
                    href="/maintenance-module"
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-smooth ${isActive("/maintenance-module")}`}
                  >
                    Maintenance
                  </Link>
                )}
                <Link
                  href="/backups"
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-smooth ${isActive("/backups")}`}
                >
                  Backups
                </Link>
                {permissions.team !== false && (
                  <Link
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-smooth ${isActive("/team-management")}`}
                  >
                    Team
                  </Link>
                )}
                {permissions.settings !== false && (
                  <Link
                    href="/settings"
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-smooth ${isActive("/settings")}`}
                  >
                    Settings
                  </Link>
                )}
                          ...
                          >
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <p
                                  className={`text-sm font-medium truncate ${!notification.read ? "text-text-primary" : "text-text-secondary"}`}
                                >
                                  {notification.title}
                                </p>
            ...
            <button
              id="mobileMenuBtn"
              className="md:hidden p-2 rounded-lg hover:bg-surface-hover transition-smooth"
        ...
        <div
          id="mobileMenu"
          className="hidden md:hidden border-t border-border bg-surface w-full"
        >
          <nav className="w-full py-[2vh] px-0 flex flex-col gap-2">
            <Link
              href="/dashboard"
              className={`px-4 py-2 rounded-lg text-sm font-medium ${isActive("/dashboard")}`}
            >
              Dashboard
            </Link>
            {permissions.clients !== false && (
              <Link
                href="/client-management"
                className={`px-4 py-2 rounded-lg text-sm font-medium ${isActive("/client-management")}`}
              >
                Clients
              </Link>
            )}
            {permissions.domains !== false && (
              <>
                <Link
                  href="/domain-management"
                  className={`px-4 py-2 rounded-lg text-sm font-medium ${isActive("/domain-management")}`}
                >
                  Domains
                </Link>
                <Link
                  href="/domain-management/vendors"
                  className={`px-4 py-2 rounded-lg text-sm font-medium ${isActive("/domain-management/vendors")} ml-4 border-l-2 border-border`}
                >
                  Vendors
                </Link>
              </>
            ...
            {permissions.hosting !== false && (
              <Link
                href="/hosting-management"
                className={`px-4 py-2 rounded-lg text-sm font-medium ${isActive("/hosting-management")}`}
              >
                Hosting
              </Link>
            )}
            {permissions.maintenance !== false && (
              <Link
                href="/maintenance-module"
                className={`px-4 py-2 rounded-lg text-sm font-medium ${isActive("/maintenance-module")}`}
              >
                Maintenance
              </Link>
            )}
            {permissions.team !== false && (
              <Link
                href="/team-management"
                className={`px-4 py-2 rounded-lg text-sm font-medium ${isActive("/team-management")}`}
              >
                Team
              </Link>
            )}
            {permissions.settings !== false && (
              <Link
                href="/settings"
                className={`px-4 py-2 rounded-lg text-sm font-medium ${isActive("/settings")}`}
              >
                Settings
              </Link>
            )}
